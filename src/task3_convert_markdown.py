...
def convert_legal_docs():
    ...
    if not legal_dir.exists():
        print("  (bỏ qua: data/landing/legal/ chưa tồn tại)")
        return
...
def convert_news_articles():
    ...
    Task 2 (task2_crawl_news.py) đã crawl trực tiếp ra .md với YAML front
    matter sẵn — chỉ cần copy nguyên văn sang data/standardized/news/. File
    .json (định dạng cũ / crawler khác) vẫn được hỗ trợ để tương thích ngược.
    """
    ...
    if not news_dir.exists():
        print("  (bỏ qua: data/landing/news/ chưa tồn tại)")
        return
